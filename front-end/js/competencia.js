...
var server = 'http://localhost:8080';//'http://0.0.0.0:8080';
...
function CompetenciasController () {
	...
	this.cargarCompetencias = function (data){
		...
		var cantidad = data.cant;//data.length; //CAMBIE YO
		var idColor = 1;
		...
		for (i = 0; i < cantidad; i++) {
			...
			$(divCompetencia).find('.link').each( function(){
				$( this ).attr("href",$( this ).attr("href")+data.competencias[i].id); //LE AGREGUE .COMPETENCIAS
			});
			// Se coloca el nombre de cada competencia
			$(divCompetencia).find('.titulo').text(data.competencias[i].nombre); //LE AGREGUE .COMPETENCIAS
	...
	this.cargarCompetencia = function (id, data){			//LE AGREGUE .COMPETENCIAS A TODOS
	...
	this.cargarGeneros = function (){
		...
		$.getJSON(server+"/generos",
			function(data){
		    	...
		    	for (i = 0; i < data.generos.length; i++) { //LE AGREGUE .GENERO
						console.log((data.generos)[i].id); //ESO CAMBIE
	...
	this.cargarDirectores = function (){
		...
		$.getJSON(server+"/directores",
			function(data){
		    	...
		    	for (i = 0; i < data.directores.length; i++) { //LE AGREGUE .DIRECTORES
						console.log((data.directores)[i].id); //ESO CAMBIE
